...
from statistics import PerformWilcoxonMotion
...
for i in range(20,23):
    subdir.append('HC_'+str(i)+'/')
sequs = ['T1_MPR', 'T2_TSE', 'T1_TIRM', 'T2_FLAIR', 'T2STAR', 'TRACEW_B0']
...
if new_calc:
    for sequ in sequs:
        ...
        for mot in motion:
            ...
            print(sequ, mot)
            for subj in subdir:
                if sequ in ['TRACEW_B0', 'T2_FLAIR'] and subj in ['HC_0'+str(i)+'/' for i in range(1,10)]:
                    continue
                if sequ in ['TRACEW_B0', 'T2_FLAIR'] and subj in ['HC_'+str(i)+'/' for i in range(10,13)]:
                    continue
                ...
                magn_Off, RMS_Off, med_disp_Off, max_disp_Off = CalcMotionMetricsforScan(subj, name, seq_type)
